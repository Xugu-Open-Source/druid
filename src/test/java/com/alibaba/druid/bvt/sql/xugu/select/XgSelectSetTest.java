...
public class XgSelectSetTest extends TestCase {

    public void testIntersect() {
        String sql = "SELECT id FROM tb_top INTERSECT SELECT tid FROM tb_top_merge;";
    ...
    public void testUnion() {
        String sql = "SELECT id, name FROM tb_top UNION SELECT tid, name FROM tb_top_merge;";
        XuGuStatementParser parser = new XuGuStatementParser(sql);
        List<SQLStatement> sqlStatements = parser.parseStatementList();
        SQLStatement stmt = sqlStatements.get(0);
        XuGuSchemaStatVisitor visitor = new XuGuSchemaStatVisitor();
    ...
    public void testUnionAll() {
        String sql = "SELECT id, name FROM tb_top UNION ALL SELECT tid, name FROM tb_top_merge;";
        XuGuStatementParser parser = new XuGuStatementParser(sql);
        List<SQLStatement> sqlStatements = parser.parseStatementList();
        SQLStatement stmt = sqlStatements.get(0);
        XuGuSchemaStatVisitor visitor = new XuGuSchemaStatVisitor();
