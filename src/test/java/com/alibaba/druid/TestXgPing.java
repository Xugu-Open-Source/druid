...
//        OracleConnection oracleConn = (OracleConnection) conn;
    ...
    private void ping_1000(Connection conn) throws SQLException {
    ...
    private void select_1000(Connection conn) throws SQLException {
        long startMillis = System.currentTimeMillis();
        final int COUNT = 10000;
        for (int i = 0; i < COUNT; ++i) {
            select(conn);
    ...
    public void pring(Connection conn) throws SQLException {
        conn.isValid(1000);
    }

    public void select(Connection conn) throws SQLException {
        Statement stmt = conn.createStatement();
        stmt.execute("SELECT 1 FROM TEST1");
        stmt.close();
    }
}
