...
import com.alibaba.druid.sql.dialect.xugu.ast.XuGuPartitionSingle;
...
public class XuGuOutputVisitor extends SQLASTOutputVisitor implements XuGuASTVisitor {
    ...
    public XuGuOutputVisitor(StringBuilder appender) {
        super(appender);
    }

    public XuGuOutputVisitor(StringBuilder appender, boolean parameterized) {
...
//    }
    ...
    public boolean visit(SQLSetStatement x) {
        boolean printSet = x.getAttribute("parser.set") == Boolean.TRUE || DbType.xugu != dbType;
        if (printSet) {
            print0(ucase ? "SET " : "set ");
        }
    ...
    public boolean visit(SQLPartitionSingle x) {
        if (x instanceof XuGuPartitionSingle) {
            return visit((XuGuPartitionSingle) x);
        }
        return super.visit(x);
    }

    public boolean visit(XuGuPartitionSingle x) {
        boolean isDbPartiton = false, isTbPartition = false;
        final SQLObject parent = x.getParent();
        if (parent != null) {
            final SQLObject parent2 = parent.getParent();
            // if (parent2 instanceof XuGuCreateTableStatement) {
            //     XuGuCreateTableStatement stmt = (XuGuCreateTableStatement) parent2;
